...
describe('stringify()', function (t) {
	test('stringifies a querystring object', function (st) {
	...
	test('stringifies falsy values', function (st) {
	...
	test('stringifies symbols', function (st) {
		...
		).toBe('a[]=Symbol%28Symbol.iterator%29');
	});
	...
	test('stringifies bigints', function (st) {
		...
		expect(stringify({ a: [three] }, { encodeValuesOnly: true, arrayFormat: 'brackets' })).toBe(
			'a[]=3',
		);
		expect(
			stringify(
	...
	test('encodes dot in key of object when encodeDotInKeys and allowDots is provided', function (st) {
		expect(
			stringify(
	...
	test('should encode dot in key of object, and automatically set allowDots to `true` when encodeDotInKeys is true and allowDots in undefined', function (st) {
		// st.equal(
		// 	stringify(
		// 		{ 'name.obj.subobject': { 'first.godly.name': 'John', last: 'Doe' } },
	...
	test('should encode dot in key of object when encodeDotInKeys and allowDots is provided, and nothing else when encodeValuesOnly is provided', function (st) {
		// st.equal(
		// 	stringify(
		// 		{ 'name.obj': { first: 'John', last: 'Doe' } },
		// 		{
	...
	test('throws when `commaRoundTrip` is not a boolean', function (st) {
		// st['throws'](
		// 	function () {
	...
	test('throws when `encodeDotInKeys` is not a boolean', function (st) {
		// st['throws'](function () {
	...
	test('stringifies nested falsy values', function (st) {
		...
		expect(stringify({ a: { b: { c: null } } }, { strictNullHandling: true })).toBe(
			'a%5Bb%5D%5Bc%5D',
		);
		expect(stringify({ a: { b: { c: false } } })).toBe('a%5Bb%5D%5Bc%5D=false');
	});

	test('stringifies a nested object', function (st) {
	...
	test('`allowDots` option: stringifies a nested object with dots notation', function (st) {
	...
	test('stringifies an array value', function (st) {
		// st.equal(
	...
	test('`skipNulls` option', function (st) {
		// st.equal(
	...
	test('omits array indices when asked', function (st) {
		// st.equal(stringify({ a: ['b', 'c', 'd'] }, { indices: false }), 'a=b&a=c&a=d');
		expect(stringify({ a: ['b', 'c', 'd'] }, { indices: false })).toBe('a=b&a=c&a=d');
	});

	test('omits object key/value pair when value is empty array', function (st) {
		// st.equal(stringify({ a: [], b: 'zz' }), 'b=zz');
		expect(stringify({ a: [], b: 'zz' })).toBe('b=zz');
	});

	test('should not omit object key/value pair when value is empty array and when asked', function (st) {
		// st.equal(stringify({ a: [], b: 'zz' }), 'b=zz');
	...
	describe('stringifies an array value with one item vs multiple items', function (st) {
		test('non-array item', function (s2t) {
			// s2t.equal(
			...
			expect(stringify({ a: 'c' }, { encodeValuesOnly: true, arrayFormat: 'brackets' })).toBe(
				'a=c',
			);
		...
		test('array with a single item', function (s2t) {
			// s2t.equal(
			// 	stringify({ a: ['c'] }, { encodeValuesOnly: true, arrayFormat: 'indices' }),
			// 	'a[0]=c',
			// );
			...
			expect(stringify({ a: ['c'] }, { encodeValuesOnly: true, arrayFormat: 'indices' })).toBe(
				'a[0]=c',
			);
			expect(stringify({ a: ['c'] }, { encodeValuesOnly: true, arrayFormat: 'brackets' })).toBe(
				'a[]=c',
		...
		test('array with multiple items', function (s2t) {
			// s2t.equal(
			...
			expect(stringify({ a: ['c', 'd'] }, { encodeValuesOnly: true, arrayFormat: 'comma' })).toBe(
				'a=c,d',
			);
			expect(
				stringify(
		...
		test('array with multiple items with a comma inside', function (s2t) {
			// s2t.equal(
	...
	test('stringifies a nested array value', function (st) {
		expect(
			stringify({ a: { b: ['c', 'd'] } }, { encodeValuesOnly: true, arrayFormat: 'indices' }),
		).toBe('a[b][0]=c&a[b][1]=d');
		expect(
			stringify({ a: { b: ['c', 'd'] } }, { encodeValuesOnly: true, arrayFormat: 'brackets' }),
		).toBe('a[b][]=c&a[b][]=d');
		expect(
			stringify({ a: { b: ['c', 'd'] } }, { encodeValuesOnly: true, arrayFormat: 'comma' }),
		).toBe('a[b]=c,d');
		expect(stringify({ a: { b: ['c', 'd'] } }, { encodeValuesOnly: true })).toBe(
			'a[b][0]=c&a[b][1]=d',
		);
	});

	test('stringifies comma and empty array values', function (st) {
		// st.equal(
		...
		expect(stringify({ a: [',', '', 'c,d%'] }, { encode: false, arrayFormat: 'comma' })).toBe(
			'a=,,,c,d%',
		);
	...
	test('stringifies comma and empty non-array values', function (st) {
		// st.equal(
	...
	test('stringifies a nested array value with dots notation', function (st) {
		// st.equal(
		// 	stringify(
		// 		{ a: { b: ['c', 'd'] } },
	...
	test('stringifies an object inside an array', function (st) {
		// st.equal(
		...
		expect(stringify({ a: [{ b: 'c' }] }, { arrayFormat: 'repeat', encodeValuesOnly: true })).toBe(
			'a[b]=c',
		);
		expect(
			stringify({ a: [{ b: 'c' }] }, { arrayFormat: 'brackets', encodeValuesOnly: true }),
		...
		expect(stringify({ a: [{ b: { c: [1] } }] }, { encodeValuesOnly: true })).toBe(
			'a[0][b][c][0]=1',
		);
	});

	test('stringifies an array with mixed objects and primitives', function (st) {
		// st.equal(
		...
		// 	stringify({ a: [{ b: 1 }, 2, 3] }, { encodeValuesOnly: true, arrayFormat: 'comma' }),
		// ).toBe('???');
		expect(stringify({ a: [{ b: 1 }, 2, 3] }, { encodeValuesOnly: true })).toBe(
			'a[0][b]=1&a[1]=2&a[2]=3',
		);
	});

	test('stringifies an object inside an array with dots notation', function (st) {
		// st.equal(
		...
		expect(stringify({ a: [{ b: { c: [1] } }] }, { allowDots: true, encode: false })).toBe(
			'a[0].b.c[0]=1',
		);
	});

	test('does not omit object keys when indices = false', function (st) {
		// st.equal(stringify({ a: [{ b: 'c' }] }, { indices: false }), 'a%5Bb%5D=c');
		expect(stringify({ a: [{ b: 'c' }] }, { indices: false })).toBe('a%5Bb%5D=c');
	});

	test('uses indices notation for arrays when indices=true', function (st) {
	...
	test('uses indices notation for arrays when no arrayFormat is specified', function (st) {
		// st.equal(stringify({ a: ['b', 'c'] }), 'a%5B0%5D=b&a%5B1%5D=c');
		expect(stringify({ a: ['b', 'c'] })).toBe('a%5B0%5D=b&a%5B1%5D=c');
	});

	test('uses indices notation for arrays when arrayFormat=indices', function (st) {
	...
	test('uses repeat notation for arrays when arrayFormat=repeat', function (st) {
		// st.equal(stringify({ a: ['b', 'c'] }, { arrayFormat: 'repeat' }), 'a=b&a=c');
		expect(stringify({ a: ['b', 'c'] }, { arrayFormat: 'repeat' })).toBe('a=b&a=c');
	});

	test('uses brackets notation for arrays when arrayFormat=brackets', function (st) {
	...
	test('stringifies an empty array in different arrayFormat', function (st) {
		...
		expect(stringify({ a: [], b: [null], c: 'c' }, { encode: false, arrayFormat: 'repeat' })).toBe(
			'b=&c=c',
		);
		expect(stringify({ a: [], b: [null], c: 'c' }, { encode: false, arrayFormat: 'comma' })).toBe(
			'b=&c=c',
		);
		expect(
			stringify(
	...
	test('stringifies a null object', { skip: !Object.create }, function (st) {
	...
	test('returns an empty string for invalid input', function (st) {
	...
	test('drops keys with a value of undefined', function (st) {
		...
		expect(stringify({ a: { b: undefined, c: null } }, { strictNullHandling: true })).toBe(
			'a%5Bc%5D',
		);
		expect(stringify({ a: { b: undefined, c: null } }, { strictNullHandling: false })).toBe(
			'a%5Bc%5D=',
		);
		expect(stringify({ a: { b: undefined, c: '' } })).toBe('a%5Bc%5D=');
	});

	test('url encodes values', function (st) {
		// st.equal(stringify({ a: 'b c' }), 'a=b%20c');
		expect(stringify({ a: 'b c' })).toBe('a=b%20c');
	});

	test('stringifies a date', function (st) {
		var now = new Date();
	...
	test('stringifies the weird object from qs', function (st) {
		// st.equal(
	...
	test('skips properties that are part of the object prototype', { skip: true }, function (st) {
	...
	test('stringifies boolean values', function (st) {
	...
	test('stringifies buffer values', function (st) {
		// st.equal(stringify({ a: Buffer.from('test') }), 'a=test');
		// st.equal(stringify({ a: { b: Buffer.from('test') } }), 'a%5Bb%5D=test');
	});

	test('stringifies an object using an alternative delimiter', function (st) {
	...
	// test('does not blow up when Buffer global is missing', function (st) {
	...
	test('non-circular duplicated references can still work', function (st) {
		var hourOfDay = {
	...
	test('selects properties when filter=array', function (st) {
	...
	test('supports custom representations when filter=function', function (st) {
		var calls = 0;
	...
	test('can disable uri encoding', function (st) {
		...
		expect(stringify({ a: 'b', c: null }, { strictNullHandling: true, encode: false })).toBe(
			'a=b&c',
		);
	});

	test('can sort the keys', function (st) {
		// @ts-expect-error
		var sort: StringifyOptions['sort'] = function (a: string, b: string) {
	...
	test('can sort the keys at depth 3 or more too', function (st) {
		// @ts-expect-error
		var sort: StringifyOptions['sort'] = function (a: string, b: string) {
			return a.localeCompare(b);
		};
	...
	test('can stringify with custom encoding', function (st) {
		// st.equal(
		// 	stringify(
		// 		{ 県: '大阪府', '': '' },
		// 		{
	...
	test('receives the default encoder as a second argument', function (st) {
		// stringify(
	...
	test('throws error with wrong encoder', function (st) {
		// st['throws'](function () {
	...
	test('serializeDate option', function (st) {
		var date = new Date();
		// st.equal(
		// 	stringify({ a: date }),
	...
	test('RFC 1738 serialization', function (st) {
	...
	test('RFC 3986 spaces serialization', function (st) {
	...
	test('Backward compatibility to RFC 3986', function (st) {
		// st.equal(stringify({ a: 'b c' }), 'a=b%20c');
	...
	test('Edge cases and unknown formats', function (st) {
	...
	test('encodeValuesOnly', function (st) {
		// st.equal(
		// 	stringify(
		// 		{ a: 'b', c: ['d', 'e=f'], f: [['g'], ['h']] },
		// 		{ encodeValuesOnly: true, arrayFormat: 'indices' },
		...
		expect(
			stringify({ a: 'b', c: ['d', 'e'], f: [['g'], ['h']] }, { arrayFormat: 'indices' }),
		).toBe('a=b&c%5B0%5D=d&c%5B1%5D=e&f%5B0%5D%5B0%5D=g&f%5B1%5D%5B0%5D=h');
		expect(
			stringify({ a: 'b', c: ['d', 'e'], f: [['g'], ['h']] }, { arrayFormat: 'brackets' }),
		).toBe('a=b&c%5B%5D=d&c%5B%5D=e&f%5B%5D%5B%5D=g&f%5B%5D%5B%5D=h');
		expect(stringify({ a: 'b', c: ['d', 'e'], f: [['g'], ['h']] }, { arrayFormat: 'repeat' })).toBe(
			'a=b&c=d&c=e&f=g&f=h',
		);
	});

	test('encodeValuesOnly - strictNullHandling', function (st) {
		// st.equal(
	...
	test('strictNullHandling works with null serializeDate', function (st) {
	...
	test('objects inside arrays', function (st) {
		var obj = { a: { b: { c: 'd', e: 'f' } } };
		...
		expect(stringify(withArray, { encode: false, arrayFormat: 'brackets' })).toBe(
			'a[b][][c]=d&a[b][][e]=f',
		);
		expect(stringify(withArray, { encode: false, arrayFormat: 'indices' })).toBe(
			'a[b][0][c]=d&a[b][0][e]=f',
		);
		expect(stringify(withArray, { encode: false, arrayFormat: 'repeat' })).toBe(
			'a[b][c]=d&a[b][e]=f',
		);
	...
	test('stringifies sparse arrays', function (st) {
		// st.equal(
	...
	test('encodes a very long string', function (st) {
		var chars = [];
		var expected = [];
		for (var i = 0; i < 5e3; i++) {
			chars.push(' ' + i);
...
describe('stringifies empty keys', function (t) {
	empty_test_cases.forEach(function (testCase) {
		test('stringifies an object with empty string key with ' + testCase.input, function (st) {
			// st.deepEqual(
	...
	test('edge case with object/arrays', function (st) {
		...
		expect(stringify({ '': { '': [2, 3], a: 2 } }, { encode: false })).toBe(
			'[][0]=2&[][1]=3&[a]=2',
		);
